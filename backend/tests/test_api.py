...
def test_web_app_shell_and_assets_are_served_without_cache(client):
    ...
    assert release.json()["latest"] == "0.6.11"
    ...
    latest_script = client.get("/app-assets/releases/0.6.11/app.js")
    assert latest_script.status_code == 200
    assert "card-minimal-content" in latest_script.text
    assert "card-minimal-number" in latest_script.text
    assert "card-visa-mark" in latest_script.text
