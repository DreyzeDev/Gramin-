...
function createCardModal() {
  const designs = [
    ["graphite", "Чёрная", "Графитовая классика", "◒"],
    ["bronze", "Бронзовая", "Тёплый металл", "◐"],
    ["gold", "Золотая", "Выбор с блеском", "✦"]
  ];
  openModal("Выберите карту", `
    <p class="issuance-lead">Проведите по коллекции и выберите дизайн. Карта будет одна — её стиль останется вашим.</p>
    <div class="design-rail" role="radiogroup" aria-label="Дизайн карты">${designs.map(([id, title, note, glyph], index) => `
      <button type="button" class="design-choice ${index === 0 ? "selected" : ""}" data-design-choice="${id}" role="radio" aria-checked="${index === 0}">
        <span class="design-preview ${id}"><span class="design-preview-brand">GRAMIN <i>VISA</i></span><span class="design-preview-glyph">${glyph}</span></span>
        <b>${title}</b><small>${note}</small>
      </button>`).join("")}</div>
    <div class="issuance-form"><label>Валюта<select name="currency"><option>AZN</option><option>USD</option><option>EUR</option><option>RUB</option></select></label>
    <input name="design" type="hidden" value="graphite">
    <label>PIN<input name="pin" type="password" inputmode="numeric" pattern="[0-9]{4}" maxlength="4" placeholder="4 цифры" required></label></div>`, async data => {
