...
function showShell() {
  $("#boot").classList.add("hidden"); $("#auth").classList.add("hidden"); $("#shell").classList.remove("hidden"); $("#shell").classList.add("home-active");
  document.querySelector('meta[name="theme-color"]').content = "#080909";
}
...
function render() {
  ...
  if (state.profile) {
    ...
    $("#home-greeting").textContent = state.profile.first_name || "Gramin";
  }
}
...
function renderHomeCardControl() {
  ...
  const lastFour = String(card.number || "").replace(/\D/g, "").slice(-4);
  host.innerHTML = `<button type="button" class="home-card-featured ${escapeHTML(card.design)} ${card.frozen ? "frozen" : ""}" data-home-card-open aria-label="Открыть карту"><span class="home-featured-brand">GRAMIN <i>VISA</i></span><span class="home-featured-chip"></span><b>•••• •••• •••• ${lastFour}</b><small>${name} · ${card.frozen ? "Заморожена" : "Активна"}</small></button>`;
...
function createCardModal() {
  ...
  openModal("Выберите карту", `
    <div class="issuance-hero"><button type="button" class="issuance-close" id="issuance-close" aria-label="Назад">‹</button><span>Виртуальная карта</span><h4 id="issuance-title">Gramin Black</h4><p id="issuance-price">Бесплатный выпуск</p></div>
    <p class="issuance-lead">Листайте коллекцию, чтобы выбрать карту. Доступно до двух карт на аккаунт.</p>
  ...
  const form = $("#modal-form");
  $("#issuance-close", form).onclick = () => $("#modal").close();
  $$('[data-design-choice]', form).forEach(choice => choice.onclick = () => {
    ...
    $("#issuance-title", form).textContent = `Gramin ${design[1]}`;
    $("#issuance-price", form).textContent = design[0] === "gold" ? "Премиальный выпуск" : "Бесплатный выпуск";
...
function setView(name) {
  ...
  document.querySelector('meta[name="theme-color"]').content = name === "home" ? "#080909" : "#f3f3f3";
...
function bind() {
  $$('[data-auth-mode]').forEach(button => button.onclick = () => {
    ...
    $("#auth").classList.toggle("registering", button.dataset.authMode === "register");
    $("#auth-title").textContent = button.dataset.authMode === "register" ? "Регистрация" : "Вход";
  });
  ...
  $("[data-home-analytics]").onclick = () => setView("analytics");
