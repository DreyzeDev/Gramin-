...
function renderHomeCardControl() {
  ...
  if (!card) {
    host.innerHTML = `<button class="home-card-empty" data-action="create-card">${icon("card")}<span><b>Выпустите карту</b><small>Она появится здесь вместе с управлением</small></span>${icon("plus")}</button>`;
...
function createCardModal() {
  ...
  openModal("Выберите карту", `
    ...
    <p class="issuance-lead">Проведите по коллекции и выберите дизайн. Для аккаунта доступна одна виртуальная карта.</p>
