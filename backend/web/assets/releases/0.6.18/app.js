...
function renderHomeCardControl() {
  ...
  host.innerHTML = `<div class="home-card-panel">
      <button type="button" class="home-card-summary" data-home-card-open aria-label="Открыть управление картой">
        <span>Карта ${name}</span><strong>${money(balance, card.currency)}</strong><small>${card.frozen ? "Заморожена" : "Активна"} · •••• ${lastFour}</small>
      </button>
      <button type="button" class="home-card-mini ${escapeHTML(card.design)} ${card.frozen ? "frozen" : ""}" data-home-card-open aria-label="Открыть карту"><span class="home-card-brand">GRAMIN <i>VISA</i></span><b>•••• ${lastFour}</b><small>${name}</small></button>
    </div>
    <div class="home-card-actions">
      <button type="button" data-home-card-freeze>${icon("snowflake")} ${card.frozen ? "Разморозить" : "Заморозить"}</button>
      <button type="button" data-home-card-open>Управлять ${icon("down")}</button>
    </div>`;
