...
function renderCards() {
  ...
  const initials = state.profile ? `${state.profile.first_name[0] || ""}${state.profile.last_name[0] || ""}`.toUpperCase() : "G";
  const handle = state.profile ? `@${state.profile.username}` : "@gramin";
        ...
        <button type="button" class="bank-card ${escapeHTML(card.design)} ${card.frozen ? "frozen" : ""}" data-card-flip="${card.id}" aria-label="Перевернуть карту" aria-pressed="false">
          <span class="card-inner">
            <span class="card-face card-front">
              ...
              <span class="personal-mark" aria-hidden="true">${escapeHTML(initials)}</span>
              <span class="card-top"><span class="card-brand"><i>G</i><b>GRAMIN</b></span><span class="card-type">GRAMIN ID</span></span>
              ...
              <span class="card-bottom"><span><small>ВЛАДЕЛЕЦ</small><b>${escapeHTML(holder)}</b><em>${escapeHTML(handle)}</em></span><span><small>ДЕЙСТВУЕТ ДО</small><b>${card.expiry}</b></span><strong>${card.currency}</strong></span>
            </span>
            <span class="card-face card-back" hidden aria-hidden="true" inert>
              <span class="card-minimal-band" aria-hidden="true"></span>
              <span class="card-minimal-content">
                <span class="card-minimal-number">${formatCardNumber(card.number)}</span><span class="card-copy" aria-hidden="true"></span>
                <span class="card-minimal-meta"><span><b>${card.expiry}</b><small>СРОК</small></span><span><b>${card.cvv}</b><small>CVV</small></span></span>
              </span>
            </span>
          </span>
        </button>
